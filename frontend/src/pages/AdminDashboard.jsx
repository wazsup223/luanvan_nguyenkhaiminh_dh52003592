...
const AdminDashboard = ({ user }) => {
  ...
  const updatePaymentStatus = async (orderId, paymentStatus, paymentMethod) => {
    try {
      ...
      const data = await res.json();
      if (data.success) loadAllData();
    } catch (err) { alert('Lỗi cập nhật thanh toán'); }
  };

  const todayOrders = orders.filter(o => new Date(o.created_at).toDateString() === new Date().toDateString());
  const totalRevenue = orders.filter(o => o.payment_status === 'paid')
    .reduce((sum, o) => sum + parseFloat(o.subtotal||0) - parseFloat(o.discount_amount||0) + parseFloat(o.tax_amount||0), 0);
    ...
    orders.forEach(order => {
      if (order.order_items) {
        order.order_items.forEach(item => {
          const name = item.menu_item?.item_name || 'Unknown';
          if (!itemCounts[name]) itemCounts[name] = { name, qty: 0 };
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg text-center max-w-md">
        ...
        <div className="bg-gray-100 p-4 rounded text-left text-sm">
          ...
          <p>2. Backend đang chạy port 3001?</p>
          <p className="mt-2 text-xs">npm start (trong backend/)</p>
        </div>
        <button onClick={loadAllData} className="mt-4 bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700">
          Thử lại
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-red-600 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-black">FastFood Admin</h1>
              <p className="text-sm text-red-200">Hệ thống quản lý đa chi nhánh</p>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <a href="/" target="_blank" className="hover:text-yellow-400 text-sm">🍽️ Xem Website</a>
            <button onClick={loadAllData} className="bg-white text-red-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-100">
              🔄 Làm mới
            </button>
            {user && (
              <div className="flex items-center gap-3 border-l border-red-400 pl-4">
                ...
                <div className="w-9 h-9 bg-white text-red-600 rounded-full flex items-center justify-center font-bold text-sm shadow">
                  {(user.full_name || user.username || 'A')[0].toUpperCase()}
                </div>
                <button
                  onClick={() => {
                    ['fastfood_user','fastfood_token','fastfood_userId','fastfood_userRole'].forEach(k => localStorage.removeItem(k));
                    navigate('/');
                  }}
        ...
        {activeTab === 'dashboard' && (
          <div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-yellow-500">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-yellow-600">{totalRevenue.toLocaleString('vi-VN')}đ</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                {getTopItems().length > 0 ? (
                  <Bar data={topItemsData} options={{ indexAxis: 'y', responsive: true, plugins: { legend: { display: false } } }} />
                ) : (
            ...
            {lowStockItems.length > 0 && (
              <div className="bg-white rounded-xl shadow-md p-6 border-2 border-orange-300 mb-8">
                    ...
                    {orders.slice(0, 5).map(order => (
                      <tr key={order.order_id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded text-xs ${order.payment_status === 'paid' ? 'bg-green-100 text-yellow-800' : 'bg-yellow-100 text-yellow-800'}`}>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {orders.map(order => (
                      <tr key={order.order_id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 font-medium text-red-600">#{order.order_id}</td>
                        ...
                        <td className="px-6 py-4">
                          <details className="text-sm">
                            <summary className="cursor-pointer text-red-600 hover:underline">Chi tiết</summary>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-yellow-100 rounded-lg p-4">
                <p className="text-sm text-yellow-700">✅ Bình thường</p>
              ...
              <div className="bg-red-100 rounded-lg p-4">
                <p className="text-sm text-red-700">💰 Giá trị tồn kho</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {inventory.map(item => (
                <div key={item.inventory_id} className={`bg-white rounded-lg shadow p-4 ${item.quantity <= item.min_threshold ? 'border-2 border-orange-400' : ''}`}>
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-semibold text-gray-800">{item.item_name}</h3>
                    <span className={`px-2 py-1 rounded text-xs ${item.quantity <= item.min_threshold ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-yellow-800'}`}>
                      ...
                      <tr key={user.user_id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded text-xs ${
                            ...
                            user.role === 'Customer' ? 'bg-yellow-100 text-yellow-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>{getRoleText(user.role)}</span>
                        </td>
                        <td className="px-6 py-4 text-gray-600">{user.branch?.branch_name || '-'}</td>
                        <td className="px-6 py-4 font-semibold text-red-600">{user.points || 0} pts</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded text-xs ${user.is_active ? 'bg-green-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                    ...
                    return (
                      <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                        <div className="w-full rounded-t-md transition-all hover:opacity-80"
                          style={{ height: `${heightPct}%`, background: 'linear-gradient(to top, #E4002B, #FFB81C)', minHeight: val > 0 ? '4px' : '0px' }} />
              ...
              {expenseData ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-red-50 rounded-lg p-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-red-800">{(parseInt(expenseData.total_revenue)||0).toLocaleString('vi-VN')}đ</p>
                  </div>
                  <div className="bg-red-50 rounded-lg p-4 text-center">
                    <p className="text-sm text-red-600">Expenses</p>
                    <p className="text-2xl font-bold text-red-800">{(parseInt(expenseData.total_expenses)||0).toLocaleString('vi-VN')}đ</p>
                  </div>
                  <div className={`rounded-lg p-4 text-center ${(parseInt(expenseData.net_profit)||0) >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                  ...
                  {reviews.filter(r => !r.is_approved).map(review => (
                    <div key={review.review_id} className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start justify-between gap-4">
                      ...
                      <div className="flex gap-2">
                        <button onClick={async () => { await fetch(`${API_BASE}/reviews/${review.review_id}/approve`, { method: 'PUT' }); loadAllData(); }}
                          className="px-4 py-2 bg-green-600 text-white text-sm font-bold rounded-lg hover:bg-green-700">✅ Duyệt</button>
                        <button onClick={async () => { await fetch(`${API_BASE}/reviews/${review.review_id}/reject`, { method: 'PUT' }); loadAllData(); }}
                          className="px-4 py-2 bg-red-500 text-white text-sm font-bold rounded-lg hover:bg-red-600">🗑️ Xóa</button>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {reviews.map(review => (
                      <tr key={review.review_id} className={review.is_approved ? '' : 'bg-yellow-50'}>
                        ...
                        <td className="px-4 py-3">
                          {review.is_approved
                            ? <span className="px-2 py-1 bg-green-100 text-yellow-700 rounded-full text-xs font-bold">Đã duyệt</span>
                        ...
                        <td className="px-4 py-3">
                          {!review.is_approved && (
                            <button onClick={async () => { await fetch(`${API_BASE}/reviews/${review.review_id}/approve`, { method: 'PUT' }); loadAllData(); }}
                              className="text-green-600 hover:text-green-800 font-semibold">Duyệt</button>
                          )}
                        </td>
                      </tr>
                    ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {promotions.map(promo => (
                <div key={promo.promotion_id} className={`bg-white rounded-lg shadow p-5 ${promo.is_active ? 'border-l-4 border-yellow-500' : 'border-l-4 border-gray-300 opacity-60'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-semibold text-gray-800">{promo.promotion_name}</h3>
                    <span className={`px-2 py-1 rounded text-xs ${promo.is_active ? 'bg-green-100 text-yellow-800' : 'bg-gray-100 text-gray-500'}`}>
                      {promo.is_active ? '✅ Active' : '❌ Inactive'}
                    </span>
                  </div>
                  <p className="text-sm text-red-600 font-bold mb-2">{promo.promotion_code}</p>
                  <div className="flex justify-between text-sm text-gray-600 mb-3">
                    <span>{promo.discount_type === 'percentage' ? `${promo.discount_value}%` : `${promo.discount_value.toLocaleString('vi-VN')}đ`}</span>
                  ...
                  <div className="text-xs text-gray-500">
                    ...
                    {promo.min_order_amount > 0 && <p>Đơn tối thiểu: {promo.min_order_amount.toLocaleString('vi-VN')}đ</p>}
                  </div>
                </div>
              ))}
              {promotions.length === 0 && (
              ...
              <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  <div className="bg-red-100 rounded-lg p-4 text-center">
                    ...
                    <p className="text-xl font-bold text-red-800">{parseInt(cogsData.total_revenue).toLocaleString('vi-VN')}đ</p>
                  </div>
                  <div className="bg-red-100 rounded-lg p-4 text-center">
                  ...
                  <div className="bg-yellow-100 rounded-lg p-4 text-center">
                    <p className="text-sm text-yellow-600">Lợi nhuận gộp</p>
                    <p className="text-xl font-bold text-yellow-800">{parseInt(cogsData.gross_profit).toLocaleString('vi-VN')}đ</p>
                  </div>
                  <div className="bg-red-100 rounded-lg p-4 text-center">
                    <p className="text-sm text-red-600">Biên lợi nhuận</p>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {cogsData.by_item.map((item, idx) => (
                          <tr key={idx}>
                            ...
                            <td className="px-4 py-2 text-right text-yellow-600 font-semibold">{parseInt(item.revenue - item.cogs).toLocaleString('vi-VN')}đ</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {/* Toast notification */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-xl shadow-2xl font-bold text-white max-w-sm ${
          toast.type === 'success' ? 'bg-green-500' : toast.type === 'warning' ? 'bg-yellow-500 text-gray-900' : 'bg-red-600'
        }`}>
          {toast.message}
        </div>
      )}

  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError('Không thể tải dữ liệu. Vui lòng thử lại sau. Lỗi: ' + (err.message || 'Unknown'));
      } finally {
        setLoading(false);
      }
    };
              ...
              <div key={item.item_id || i} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-red-100 hover:shadow-xl hover:shadow-red-100 hover:-translate-y-2 transition-all duration-300 cursor-pointer">
                ...
                <div className="p-4">
                  <h3 className="font-bold text-gray-900 text-sm leading-tight mb-1 group-hover:text-red-600 transition-colors line-clamp-2">
                    {item.item_name || item.name || `Món ăn #${i + 1}`}
                  </h3>
                  {item.category_name && (
              ...
              <div key={branch.branch_id} className="bg-white rounded-2xl p-6 border border-gray-100 hover:border-red-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-200">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    {branch.phone && <p className="text-sm text-gray-400 mt-1">📞 {branch.phone}</p>}
                    <div className="mt-3 flex items-center gap-2">
                      <span className="inline-block w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                      <span className="text-xs font-semibold text-green-600">Đang mở cửa</span>
                    </div>
                  </div>
                </div>
              </div>
...
const WHY_US = [
  { icon: '🚀', title: 'Giao Nhanh', desc: '30 phút hoặc hoàn tiền – giao hàng nhanh nhất khu vực' },
  { icon: '🍽️', title: 'Đồ ăn Tươi', desc: 'Nguyên liệu được chọn lọc, chế biến ngay khi bạn đặt' },
...
const MOCK_FOODS = [
  ...
  { item_id: 5, item_name: 'Khoai Tây Chiên Lớn', category_name: 'Snack', price: 35000 },
